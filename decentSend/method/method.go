...
func Invoke(){
    t := &transmit{
        "2.0",
        "invoke",
        map[string]interface{}{"chaincodeID" : map[string]interface{}{"name":"123"},"ctorMsg" : map[string]interface{}{"function" : "invoke","args" : []string{"a", "b", "1"}},"secureContext":"jim"},RandomId()}
    b, err := json.Marshal(t)
    if err != nil {
        fmt.Println("error raised: %v", err)
    }
...
func Query() bool {
    t := &transmit{
        "2.0",
        "query",
        map[string]interface{}{"chaincodeId" : map[string]interface{}{"name":"123"},"ctorMsg":map[string]interface{}{"function" : "query", "args" : []string{"a"}},"secureContext":"jim"},RandomId()}
    b, err := json.Marshal(t)
    if err != nil {
        fmt.Println("error raised: %v", err)
    }
